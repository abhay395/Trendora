...
export default function AddressSection() {
  ...
  return (
    <div className="w-full px-5 py-6 rounded-xl border border-gray-200 space-y-6">
      ...
      {addresses.map(addr => (
        <div key={addr._id} className="flex items-start space-x-3 relative">
          <input
            type="radio"
            name="address"
            id={addr._id}
      ...
      {showForm && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-4 border-t border-gray-300">
          <div className="flex flex-col">
            ...
            />
            {errors?.name ? <p className="font-semibold text-sm text-red-600 mt-2">{errors?.phone.message}</p> : null}
          </div>
          <div className="flex flex-col">
